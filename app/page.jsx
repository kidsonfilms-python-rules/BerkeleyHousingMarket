...
export default function Home() {
  ...
  const knownSelectedDisputesRef = useRef(new Set());
  ...
  async function loadArbitrationCases() {
    ...
    const openCases = cases.filter((item) => !item.resolved);
    setArbitrationCases(openCases);
    return openCases;
  }

  useEffect(() => {
    if (!arbitratorOpen || !account) return undefined;
    let cancelled = false;
    const refresh = async () => {
      try {
        const cases = await loadArbitrationCases();
        if (cancelled || !cases) return;
        const selected = cases.filter((item) => item.selected).map((item) => String(item.id));
        const newlySelected = selected.filter((id) => !knownSelectedDisputesRef.current.has(id));
        knownSelectedDisputesRef.current = new Set(selected);
        if (newlySelected.length) setStatus(`You were selected for dispute #${newlySelected.join(", #")}. Review its private evidence and vote before the deadline.`);
      } catch { /* The console already shows contract/RPC errors when actions are attempted. */ }
    };
    refresh();
    const timer = setInterval(refresh, 30_000);
    return () => { cancelled = true; clearInterval(timer); };
  }, [arbitratorOpen, account]);
      ...
      <div className="main">
        ...
        <aside className={`side-panel ${selectedProperty ? "details-open" : ""} ${sellerOpen ? "seller-open" : ""}`}>
          ...
          {sellerOpen ? <div className="seller-view">
            ...
            {vaultItems.length > 0 && <div className="seller-vault"><div className="section-head"><h3 className="section-title">Your committed deliveries</h3><span className="section-count">{vaultItems.length}</span></div>{vaultItems.map((item) => <div className="vault-item" key={item.reportId}><div><span className="meta-label">Report #{item.reportId}</span><strong>{item.property}</strong><span>{item.type} · encrypted package committed before purchase</span></div></div>)}</div>}
            ...
            <p className="seller-intro">Put down a refundable stake to help decide buyer complaints. If selected, register your private review key, read the parties’ statements, commit a vote before the first deadline, then reveal the same vote after it.</p>
            ...
            {disputeForm.id && <p className="seller-intro">Only selected panel wallets can read this evidence or vote. Missing the reveal deadline can reduce your stake.</p>}

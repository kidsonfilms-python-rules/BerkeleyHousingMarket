...
const ABI = [
  ...
  "function corroborateReport(uint256)", "function registerArbitrator() payable", "function setArbitratorEncryptionKey(string)", "function withdrawArbitratorStake(uint256)", "function commitVote(uint256,bytes32)", "function revealVote(uint256,bool,bytes32)", "function resolveDispute(uint256)", "function resolveUnrevealedDispute(uint256)", "function claimArbitratorReward(uint256)",
...
const context = () => { const rpcUrl = process.env.RPC_URL || process.env.NEXT_PUBLIC_RPC_URL; const address = process.env.CONTRACT_ADDRESS || process.env.NEXT_PUBLIC_CONTRACT_ADDRESS; if (!rpcUrl || !ethers.isAddress(address)) throw new Error("marketplace contract is not configured"); const provider = new ethers.JsonRpcProvider(rpcUrl); return { address, provider, contract: new ethers.Contract(address, ABI, provider) }; };
...
export async function POST(request) {
  try {
    ...
    else { const sellerValid = b.sellerValid === true || b.sellerValid === "true"; const map = { confirmDelivery:["confirmDelivery",[int(b.reportId,"reportId"),b.ciphertextHash,b.key]], dispute:["openDispute",[int(b.reportId,"reportId")]], refund:["claimRefund",[int(b.reportId,"reportId")]], truthBond:["claimTruthBond",[int(b.reportId,"reportId")]], corroborate:["corroborateReport",[int(b.reportId,"reportId")]], registerArbitrator:["registerArbitrator",[]], setArbitratorEncryptionKey:["setArbitratorEncryptionKey",[b.publicKey]], withdrawArbitrator:["withdrawArbitratorStake",[int(b.amountWei,"amountWei")]], commitVote:["commitVote",[int(b.disputeId,"disputeId"),b.commitment]], revealVote:["revealVote",[int(b.disputeId,"disputeId"),sellerValid,b.salt]], resolveDispute:["resolveDispute",[int(b.disputeId,"disputeId")]], resolveTimeout:["resolveUnrevealedDispute",[int(b.disputeId,"disputeId")]], claimArbitratorReward:["claimArbitratorReward",[int(b.disputeId,"disputeId")]] }; if (!map[b.action]) throw new Error("unsupported action"); if (b.action === "setArbitratorEncryptionKey" && (typeof b.publicKey !== "string" || b.publicKey.length === 0 || b.publicKey.length > 256)) throw new Error("publicKey must be a non-empty string up to 256 characters"); if (b.action === "revealVote" && ![true, false, "true", "false"].includes(b.sellerValid)) throw new Error("sellerValid must be boolean"); [method,args] = map[b.action]; if (b.action === "dispute") value = ethers.parseEther("0.001"); if (b.action === "registerArbitrator") value = ethers.parseEther("0.01"); }
